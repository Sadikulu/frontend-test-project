...
describe("completed functionality", () => {
  it("task should not have completed class it is initiality created", () => {
    render(<TodosPage />);
    addTask("Hi there!");
...
describe("deleting functionality", () => {
  it("task sholud have been removed when clicking delete button", () => {
    render(<TodosPage />);
    addTask("Hi there!");
    //Sil butonuna basılınca confirm fonksiyonunu
    //mocking yaparak hep true gelmesini sağladık
    global.confirm = () => true;
    const listItemEl = screen.getByText(/Hi there!/i);
